...
import {
  getZoomLevelName,
  getZoomLevelValue,
  supportedViewZoomLevels,
} from "~/common/hooks/use-dashboard-workflow/useWorkflowCustomViewport";

const ActionButtonZoom: FC = () => {
  ...
  const isDisabledZoom = useMemo<{ decrease: boolean; increase: boolean }>(() => {
    const ascZoomValue = supportedViewZoomLevels.map((name) => getZoomLevelValue(name)).sort();
  ...
  return (
    <>
      ...
      >
        {supportedViewZoomLevels.map((levelName) => (
          <MuiMenuItem
            key={levelName}
...
export default memo(ActionButtonZoom);
